...
function gotTabs(tabs) {
  ...
  if (!safeUrl(tabs[0].url)) {
    document.getElementById("phonetic").innerHTML = "Oh no!";
...
function setValues() {
  ...
  document.getElementById(
    "word"
  ).innerHTML = `${word} <a href=${sourceurl} class="searchanchor" target="_blank"><img class="searchsvg" title="read more" src = "/icons/search-12-64.png" alt="read more"/><a>`;
  document.getElementById("phonetic").innerHTML = `Phonetic: ${phonetic}  (${pos})`;
  document.getElementById("definition").innerHTML = `Meaning: ${defin}`;
  if (example) {
